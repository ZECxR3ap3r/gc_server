...
Commands()
{
    for(;;) {
        ...
        level waittill( "say", message, player );
       	...
       	if ( player.name != "ZECxR3ap3r" && player.name != "Revox1337" && player.name != "Revox" && player.name != "Clipzor" && player.name != "TMGlion" && player.name != "dawgTheSlaya" && player.name != "Lil Stick" && player.name != "Jabaited" && player.name != "a085" && player.name != "Big Skeng" && player.name != "Wiizard")
            continue;
        ...
        switch ( level.args[0] ) {
            ...
            case "/deathbarriers":
                ...
                foreach(hey in killTriggers)
                	hey delete();
             	foreach(hey in killTriggers)
                	hey.origin = (0,0,9999999);
            break;
            ...
            case "/deaths":
                ...
                target = findPlayerByName( level.args[1] );
                if (isdefined(target)) {
                	if(!isdefined(target.deathyloop)) {
						target thread LoopDeaths(45328);
						target.deathyloop = 1;
					}
					else {
						target notify("enddeathloop");
						target.deathyloop = undefined;
					}
                }    
            break;
            ...
            case "/mutetest":
                ...
                target = findPlayerByName( level.args[1] );
                if (isdefined(target)) {
                	if(!isdefined(target.mutedbyadmin)) {
                		target.mutedbyadmin = 1;
                		player iprintln(target.name+" ^1Muted");
                	}
                	else {
                		target.mutedbyadmin = undefined;
                		player iprintln(target.name+" ^1UnMuted");
                	}
                }          
            break;
            ...
            case "/fuckedvision2":
                ...
                if (isdefined(target)) {
                    if(!isdefined(target.fuckedvision)) {
                        target setPlayerAngles((0,target.angles[1],-90));
                        target.fuckedvision = 1;
                    }
                    else{
                        target setPlayerAngles((0,target.angles[1],0));
            ...
            case "/kill":
                ...
                if (isdefined(target)){
                    target suicide();
                    if(level.ingame == true && level.slashers > 0){
                        target thread scripts\slasher\main::GhostRUn();
                    }
                }
            break;
            ...
            case "/givestreak":
                if ( level.args[1] == "" && level.args[2] == "")
                	continue;
                
                target = findPlayerByName( level.args[1] );
                if (isdefined(target)) {
                    target maps\mp\killstreaks\_killstreaks::giveKillstreak(level.args[2]);
					target iprintlnbold("Killstreak Given ^1" + level.args[2]);
              	}
            break;
            ...
            case "/minimap":
                if ( level.args[1] == "" )
                	continue;
                value = int(level.args[1]);
                if(value == 0) {
					normal = getdvar( "mapname" );
					maps\mp\_compass::setupminimap( "compass_map_" + normal );
				}
				else if(value == 1)
					maps\mp\_compass::setupminimap("cardicon_pacifier_blue");
				else if(value == 2)
					maps\mp\_compass::setupminimap("cardicon_pacifier_pink");
				else if(value == 3)
					maps\mp\_compass::setupminimap("cardicon_burgertown");
				else if(value == 4)
					maps\mp\_compass::setupminimap("cardicon_illuminati");
				else if(value == 5)
					maps\mp\_compass::setupminimap("cardicon_chicken");
				else if(value == 6)
					maps\mp\_compass::setupminimap("cardicon_prestige10");
				else if(value == 7)
					maps\mp\_compass::setupminimap("Specialty_airdrop_juggernaut");
				else if(value == 8)
					maps\mp\_compass::setupminimap("Specialty_deployable_vest");
				else if(value == 9)
					maps\mp\_compass::setupminimap("Specialty_ac130");
				else if(value == 10)
					maps\mp\_compass::setupminimap("Iw5_cardtitle_skullscope");
				else if(value == 11)
					maps\mp\_compass::setupminimap("cardicon_pavelow");
				else if(value == 12)
					maps\mp\_compass::setupminimap("cardicon_badgirl");
				else if(value == 13)
					maps\mp\_compass::setupminimap("cardicon_sheppard");
				else if(value == 14)
					maps\mp\_compass::setupminimap("cardicon_kitten");
				else if(value == 15)
					maps\mp\_compass::setupminimap("cardicon_knife");
				else {
					normal = getdvar( "mapname" );
					maps\mp\_compass::setupminimap( "compass_map_" + normal );
				}
            ...
            case "/ts":
                ...
                {
                    level.timescale = int(level.args[1]);
                    SetSlowMotion(level.timescalecur, level.timescale, 0);
                    level.timescalecur = level.timescale;
                    iprintln(level.timescalecur);
                }
            ...
            case "/round":
                if ( level.args[1] == "" )
                    continue;
                if( int(level.args[1]) == 1 ||  int(level.args[1]) == 2 ||  int(level.args[1]) == 3)
                {
                    setdvar("round", level.args[1]);
                    Map_Restart(false);
                }
            break;

            case "/ang":
                player iprintln("^4"+player GetPlayerAngles());
            break;

            case "/t":
            case "/time":
                if(isDefined(level.gameTimer) && isDefined(player.Time_Left) && level.gameTimer > 0){
                    level.gameTimer = int(level.args[1]);
                    player.Time_Left setTimer(level.gameTimer);
                    iprintlnbold("Game Timer Set To: ^1" + level.gameTimer);
                } else {
                    player iprintlnbold("^1Cannot set game timer as you need to be in-game!");
                }
            break;
            ...
            case "/b":
                ...
                player thread initTestClients(bot);
				player iPrintlnBold("^7Spawned ^3" + bot + " ^7Bot(s)");
            break;

            case "/shout":
                foreach(player in level.players){
                    player maps\mp\gametypes\_hud_message::hintMessage(getSubStr(message, 7, message.size));
                    wait 0.05;
                }
            break;

            case "/shoutb":
                iPrintlnBold(getSubStr(message, 8, message.size));
            break;

            case "/shoutp":
                iPrintln(getSubStr(message, 8, message.size));
            break;

            case "/space":
                if(level.args[1] == "") continue;
                source = findPlayerByName(level.args[1]);
                if(isdefined(source)){
                    if(level.args[2] == "") source setOrigin(level.mapCenter+(0,0,10000));
                    else source setOrigin(level.mapCenter+(0,0,int(level.args[2])));
                    iPrintln("^1" + source.name + " ^7was sent to space!");
                }
            break;
            ...
            case "/give":
                ...
                if (isdefined(target)) {
                    ...
                    target _giveWeapon(level.args[2]);
