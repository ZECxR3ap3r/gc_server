    ...
    if (door.hp > 0) {
        if (self.sessionteam == "allies") {
            if (door.state == "open") {
                ...
                door moveto(door.close, 2); // moveto
                wait 2.3;
                door.state = "close";
            }
            else if (door.state == "close") {
                iprintln(self.name + " ^2Opened ^7The Door!");
                door moveto(door.open, 2); // moveto
                wait 2.3;
                door.state = "open";
            }
        }
    ...
    while(1) {
    	...
    	foreach(player in level.players) {
    		foreach(ent in center.blocks) {
    			if(distance(player.origin, ent.origin) < 65 && !player islinked()) {
    				player setorigin((player.origin[0], player.origin[1], center.origin[2] + 25));
...
monitorCrateJackingreplace() {
	...
	for(;;) {
        ...
        if(owner.sessionteam != self.sessionteam)
            say_raw("^5" + self.name + " ^3Stole ^5" + owner.name + "'s^3 Carepackage!");
