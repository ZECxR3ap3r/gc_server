...
rotateGuns() {
    ...
    level.GunNameHUD = CreateServerFontString("hudbig", .5f);  
    level.GunNameHUD SetPoint("TOP", "TOP", 250, 10);
    level.GunNameHUD SetText("^5" + get_weapon_name_conv(level.PrimaryFullName));
    level.GunNameHUD.hidewheninmenu = 1;
    level.GunNameHUD thread destroy_on_notify("game_ended");
    
    level.MainHUD = CreateServerFontString("hudbig", .5f);
    level.MainHUD SetPoint("TOP", "TOP", 180, 10);
    level.MainHUD.label = &"Next gun: ";
    level.MainHUD.hidewheninmenu = 1;
    level.MainHUD thread destroy_on_notify("game_ended");
    
    level.ChangingIn = CreateServerFontString("hudbig", .5f);
    level.ChangingIn SetPoint("TOP", "TOP", 180, 25);
    level.ChangingIn.label = &"Changing in: ";
    level.ChangingIn.hidewheninmenu = 1;
    level.ChangingIn thread destroy_on_notify("game_ended");
    
    level.SecondsNumber = CreateServerFontString("hudbig", .5f);
    level.SecondsNumber SetPoint("TOP", "TOP", 223, 25);
    level.SecondsNumber.label = &"^5";
    level.SecondsNumber.hidewheninmenu = 1;
    level.SecondsNumber thread destroy_on_notify("game_ended");
    
    level.Seconds = CreateServerFontString("hudbig", .5f);
    level.Seconds SetPoint("TOP", "TOP", 260, 25);
    level.Seconds.label = &"second(s)";
    level.Seconds.hidewheninmenu = 1;
    level.Seconds thread destroy_on_notify("game_ended");
    ...
    while(true) {
        ...
        if(passedTime == level.ChangeInterval) {
            ...
            foreach(player in level.players) {
            	if(player.team == "allies") {   
                    ...
                    if(!player isUsingRemote() && player getcurrentweapon() != "none" && player getcurrentweapon() == level.GlobalCurrentGun || player.hadgunsrotated == 0) {
                        player setspawnweapon( level.PrimaryFullName );
        ...
        level.SecondsNumber setValue(level.ChangeInterval - passedTime);
        wait 1;
    }
